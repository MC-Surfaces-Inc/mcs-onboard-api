...
router.get("/", (req, res) => {
  let sql = `
  select 
    clients.id as clientId, users.id as userId, users.sageEmployeeNumber, name, territory, status.status, createdAt, clients.updatedAt, clients.sageObjectId, firstName, lastName, email, phone, status.remindAt 
  from clients 
