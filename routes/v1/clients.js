...
router.get("/:id/profile-data", (req, res) => {
  let sql = `
    ...
    select id, type, CONCAT_WS(" ", address1, address2) address, city, state, zip from addresses where clientId=?;
    select id, name, title, phone, email from contacts where clientId=?;
