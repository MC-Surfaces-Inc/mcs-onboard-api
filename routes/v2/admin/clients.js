...
router.get("/:id/profile-data", (req, res) => {
  let sql = `
    ...
    select versionId, choice, firstName, lastName, email, phone, department, division, subdivision, territory from decision join users u on decision.userId = u.id join department on u.department = department.id where d.clientId=?;
